...
SELECT TOP 3 product_id, quantity_ordered
FROM Orders
...
SELECT shipping_company, 
       AVG(DATEDIFF(day, shipment_date, delivery_date)) AS average_delivery_time
FROM Shipments
GROUP BY shipping_company;
...
SELECT TOP 3 shipping_company, COUNT(shipment_id) AS shipment_count
FROM Shipments
GROUP BY shipping_company
...
SELECT TOP 5 s.supplier_id, s.supplier_name, SUM(p.unit_price * o.quantity_ordered) AS total_revenue
FROM Suppliers s
JOIN Orders o ON s.supplier_id = o.supplier_id
JOIN Products p ON o.product_id = p.product_id
GROUP BY s.supplier_id, s.supplier_name
...
SELECT 
    s.supplier_id, 
    s.supplier_name, 
    AVG(DATEDIFF(DAY, sh.shipment_date, sh.delivery_date)) AS avg_shipment_duration
FROM Suppliers s
JOIN Orders o ON s.supplier_id = o.supplier_id
